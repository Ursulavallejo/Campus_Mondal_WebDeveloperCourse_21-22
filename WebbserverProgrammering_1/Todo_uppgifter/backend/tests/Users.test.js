...
import TodoDataBase from "../src/data/todoDataBase.js";
        ...
        it('should expect a user to be created with a task', function (done) {
            Chai.request(app)
                ...
                .end((error, response) => {
                    ...
                    // expect(body.length).to.not.equal(4)
                    // expect(body.length).to.not.equal(3)
                    //
                    // const user = body[4]
                    //
                    // expect(user).to.be.an('object');
                    // expect(user.name).to.equal('Magnus');
                    // expect(user.id).to.equal(4);
                    // expect(user.task).to.equal('go shopping');
    ...
    describe('Testing to delete a user by name that do not exist in db (DELETE) with expect', () => {
        it('should expect a string with fail message', (done) => {
            Chai.request(app)
                ...
                .end((error, response) => {
                    expect(response.status).to.equal(200)
                    expect(response.text).to.equal(`User with name : "${newUser.name}"does not exist in database!`)
                    done()
...
const deleteUserByNameExpect = () => {
    describe('Testing and create a user with a task (POST)', () => {
        it('should expect a user to be created with a task', function (done) {
            Chai.request(app)
...
describe('TESTING USER API ALIVE ROUTES', () => {
    ...
    checkIfUserDoNotExistExpect(newUser.name)// Magnus
    updateUserByNameDontExistExpect()

    createTodoExpect()
    getUserByNameExpect(newUser.id, newUser.name, newUser.task) // Magnus

    checkIfUserDoNotExistExpect(updatedUser.name)// Oliver
    updateUserByNameExpect()
    getUserByNameExpect(updatedUser.id, updatedUser.name, updatedUser.task) // Oliver

    deleteUserByNameUserDontExistExpect()
    deleteUserByNameExpect()
